...
fn empty_utreexo() {
    ...
    assert_eq!(
        forest0.root(&hasher),
        MerkleTree::empty_root(b"Rust.utreexo")
    );
}
...
fn insert_to_utreexo() {
    ...
    assert_eq!(
        forest1.root(&hasher),
        MerkleTree::root(b"Rust.utreexo", 0..6)
    );
...
fn transaction_success() {
    ...
    assert_eq!(
        ...
        MerkleTree::root(b"Rust.utreexo", &[2, 3, 4, 5, 6, 8])
    );
}
...
fn transaction_fail() {
    ...
    assert_eq!(
        new_forest.root(&hasher),
        MerkleTree::root(b"Rust.utreexo", &[2, 3, 4, 5, 1, 6])
    );
}
...
fn insert_and_delete_utreexo() {
    ...
    fn verify_update<M: MerkleItem>(
        ...
        assert_eq!(
            forest2.root(&hasher),
            MerkleTree::root(b"Rust.utreexo", new_set)
        );
